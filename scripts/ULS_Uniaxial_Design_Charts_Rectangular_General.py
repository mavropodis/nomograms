...
for d1_h in d1_h_list:
    ...
    for omegatot in omegatot_list:
        ...
        for i in ec_amount:
            ...
            for j in range(1,n+1):
                if j==1 or j==n:
                    omegaj = top_bottom*omegatot
                else:
                    omegaj = middle*omegatot
                esj = ec2+r*d1_h+(j-1)/(n-1)*r*(1-2*d1_h)
                sj_fyd = max(-1,min(1,esj/eyd))
                ns_sum += sj_fyd*omegaj
                ms_sum += sj_fyd*omegaj*(0.5-d1_h-(j-1)/(n-1)*(1-2*d1_h))
            vd = -(a*xi+ns_sum)
            md = a*xi*(0.5-z*xi)+ms_sum
            vd_list.append(vd)
            md_list.append(md)
    ...
    for es in [x/10 for x in range(-round(eyd*10),-round(eyd*10),5)]:
        critical_pairs.append((ecu2,es))
    if d1_h==0.05:
        for es in [x/10 for x in range(26,80,8)]:
            critical_pairs.append((ecu2,-es))
        for es in [x/10 for x in range(210,1020,40)]:
            critical_pairs.append((ecu2,-es))
    if d1_h==0.10:
        for es in [x/10 for x in range(26,73,8)]:
            critical_pairs.append((ecu2,-es))
        for es in [x/10 for x in range(100,470,15)]:
            critical_pairs.append((ecu2,-es))
    if d1_h==0.15:
        for es in [x/10 for x in range(28,40,6)]:
            critical_pairs.append((ecu2,-es))
        critical_pairs.append((ecu2,-4.5))
        critical_pairs.append((ecu2,-5.5))
        for es in [x/10 for x in range(40,295,10)]:
            critical_pairs.append((ecu2,-es))
    if d1_h==0.20:
        critical_pairs.append((ecu2,-2.6))
        critical_pairs.append((ecu2,-3.3))
        critical_pairs.append((ecu2,-4.0))
...
# Βρόχος μορφοποίησης και κατασκευής διαγραμμάτων αλληλεπίδρασης.
