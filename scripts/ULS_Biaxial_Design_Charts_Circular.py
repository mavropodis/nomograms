...
def rebar(strains):
    ...
    return ss_fyd
def biaxial(d1_D, omegatot, vinternal):
    # Διατομή και οπλισμός.
    bars = 8
    rsi = 0.5-d1_D
    angles = numpy.linspace(0,2*numpy.pi,bars,endpoint=False)
    xsi = 0.5+rsi*numpy.cos(angles)
    ...
    inside_circle = (xci-0.5)**2+(yci-0.5)**2<=0.5**2
    xci = xci[inside_circle]
    yci = yci[inside_circle]
    Ac = numpy.pi*(0.5**2) 
    dAc = (step*step)/Ac
    mx_list = []
    my_list = []
...
for d1_D in [0.05,0.10,0.15,0.20]:
    ...
    for octant, vexternal in enumerate(vexternal_list, start=1):
        ...
        for omegatot in omegatot_list:
            mx, my = biaxial(d1_D,omegatot,vinternal)
            if len(mx)>0:
                mx = numpy.array(mx)
                my = numpy.array(my)
                first_octant = mx>=my
    ...
    pyplot.savefig(f'plot_{d1_D}.png', format='png', dpi=300, bbox_inches='tight')